...
def assignment_1_ii():
	...
	def fit(t, p):
		...
		p1_, p2_ = 1-_p1, 1-_p2
		r = p1_/p2_
		return (1- (p1_**t + p2_**t*_p1*(r**t - 1)/(_p2-_p1))**(2*cell_count))*rb_rate
	loss = lambda p: integrate.quad(lambda t: (ecdf(t)-fit(t, p))**2, 0, 36)[0]
	p1, p2 = op.minimize(loss, [1e-7, 1e-8]).x
	print('p1 = {}, p2 = {}, relative error = {}%\n{}'\
		.format(p1, p2, 100*loss([p1, p2])**0.5/ecdf_norm, div))
	fit_plot(ecdf, lambda t: fit(t, [p1, p2]), [0, 73], 'age in months at the times of diagnosis',\
	 'probability of being diagnosed with retinoblastoma', 'two random factors')
